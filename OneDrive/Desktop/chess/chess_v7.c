...
int main()
{
    ...
    while(1) {
        ...
        if (choice == '2') {
            printf("Entrez le nom du fichier de sauvegarde à charger (ex: partie1.txt) : ");
            scanf("%s", filename);
            load_game(&current_player, filename);  // Charge l'état de la partie depuis le fichier
        } else if (choice == '1') {
            printf("Entrez le nom du fichier pour la nouvelle partie (ex: partie1.txt) : ");
            scanf("%s", filename);
            reset_save(filename);  // Réinitialise la sauvegarde pour une nouvelle partie
        }
...
again1:
    ...
    move.r2 = r2;
    move.c2 = c2;
    save_move(move, 2, filename);  // Enregistre le coup avec le joueur courant
...
again2:
    ...
    move.r2 = r2;
    move.c2 = c2;
    save_move(move, 1, filename);  // Enregistre le coup avec le joueur courant
...
// Fonction pour sauvegarder un coup avec le joueur courant dans un fichier spécifique
void save_move(Move move, int player_turn, char *filename)
{
    FILE *file = fopen(filename, "a"); // Ouvre en mode ajout dans le fichier choisi
    if (file == NULL) {
...
// Fonction pour charger la partie depuis un fichier spécifique
void load_game(int *player_turn, char *filename)
{
    FILE *file = fopen(filename, "r");  // Ouvre en mode lecture
    if (file == NULL) {
        printf("Fichier de sauvegarde non trouvé : %s\n", filename);
        return;
    }
...
// Fonction pour réinitialiser une sauvegarde (créer un fichier vide ou réécrire un fichier)
void reset_save(char *filename)
{
    FILE *file = fopen(filename, "w");  // Ouvre en mode écriture pour écraser l'existant
    if (file == NULL) {
